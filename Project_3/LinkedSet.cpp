...
template<class ItemType>
bool LinkedSet<ItemType>::add(const ItemType& new_entry) //dont change the value but access directly to save memory
{
    if(!contains(new_entry))
    {
       ...
       next_node_ptr->setItem(new_entry);	//Going to be a song
...
//Removes from left to right from beginning of chain to the end
template<class ItemType>
...
// or the null pointer if the entry is not in the set. If the entry is not in the chain it will traverse to the last node which will have a nullptr thus returns null.
template<class ItemType>
Node<ItemType>* LinkedSet<ItemType>::getPointerTo(const ItemType& an_entry) const
{
   ...
   while (!found && (cur_ptr != nullptr))	//while found != 1 && cur pt != nullptr
   {
