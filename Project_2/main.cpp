...
/*
Mistakes:
-Did playlist and Playlist not PlayList
-listOfSongs[i] messed up and did listOfSongs[i]
-	if( (isEmpty() != 0) && (located_index > -1) )
- Should be 	if( (isEmpty() != 1) && (located_index > -1) ) that was where I messed up. isEmpty() == 1 means that it is empty. But you do not want it to be empty in order to remove a song from the list

//You can call set's member functions, because playlist_ is of type Set ??????????????????????????????????????
*/
